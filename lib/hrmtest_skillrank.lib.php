...
/**
 * \file    lib/hrmtest_skillrank.lib.php
 * \ingroup hrmtest
...
function skillrankPrepareHead($object)
{
	...
	$head[$h][0] = dol_buildpath("/hrmtest/skillrank_card.php", 1).'?id='.$object->id;
	...
	if (isset($object->fields['note_public']) || isset($object->fields['note_private'])) {
		...
		$head[$h][0] = dol_buildpath('/hrmtest/skillrank_note.php', 1).'?id='.$object->id;
	...
	$upload_dir = $conf->hrmtest->dir_output."/skillrank/".dol_sanitizeFileName($object->ref);
	$nbFiles = count(dol_dir_list($upload_dir, 'files', 0, '', '(\.meta|_preview.*\.png)$'));
	$nbLinks = Link::count($db, $object->element, $object->id);
	$head[$h][0] = dol_buildpath("/hrmtest/skillrank_document.php", 1).'?id='.$object->id;
	...
	$head[$h][0] = dol_buildpath("/hrmtest/skillrank_agenda.php", 1).'?id='.$object->id;
	...
	//	'entity:-tabname:Title:@hrmtest:/hrmtest/mypage.php?id=__ID__'
	//); // to remove a tab
	complete_head_from_modules($conf, $langs, $object, $head, $h, 'skillrank@hrmtest');

	complete_head_from_modules($conf, $langs, $object, $head, $h, 'skillrank@hrmtest', 'remove');
