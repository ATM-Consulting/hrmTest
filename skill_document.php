...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->hrmtest->multidir_output[$object->entity ? $object->entity : $conf->entity]."/skill/".get_exdir(0, 0, 0, 1, $object);
}

$permissiontoadd = $user->rights->hrmtest->skill->write; // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
...
$title = $langs->trans("Skill").' - '.$langs->trans("Files");
$help_url = '';
...
if ($object->id) {
	...
	$modulepart = 'hrmtest';
	//$permissiontoadd = $user->rights->hrmtest->skill->write;
	$permissiontoadd = 1;
	//$permtoedit = $user->rights->hrmtest->skill->write;
	$permtoedit = 1;
	$param = '&id='.$object->id;
